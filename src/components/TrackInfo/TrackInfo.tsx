...
export const TrackInfo: FC<{ tabIndex?: number }> = ({ tabIndex = 0 }) => {
  const { currentTrack, isEditingTrack, trackList, userInitialized } =
  ...
  const handleKeyDown = ({ key, altKey }: KeyboardEvent) =>
    altKey && key === 'i' && toggle(TRACK_INFO_MODAL_ID)
    ...
    () => (
      <div className="mp-track-info">
        ...
        <div className="mp-track-info__metadata">
          <p className="mp-track-info__title">
            {currentTrack && currentTrack.title}
          </p>
          <p className="mp-track-info__artist">
