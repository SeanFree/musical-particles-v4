...
export const PlaylistMenu: FC = () => {
  const { isOpen, close, open, toggle } = useContext(MenuContext)
  const {
    addTracks,
    isEditingTrack,
    isPlaying,
  ...
  const trackLabel = useRef<HTMLLabelElement>(null)
  const handleKeyDown = ({ key, altKey }: KeyboardEvent) =>
    !isEditingTrack && altKey && key === 'p' && toggle(PLAYLIST_MENU_ID)

  const handleFileInput = (e: ChangeEvent) =>
    addTracks([...(e.target as HTMLInputElement).files!])
  ...
  const loadStarterTracks = () => {
    ...
    Promise.all(
      ...
      .then((files: Blob[]) => addTracks(files))
      .then(() => open(PLAYLIST_MENU_ID))
  }

  const handleLabelKeyDown: KeyboardEventHandler = (e) => {
  ...
  return useMemo(
    () => (
      <SliderMenu className="mp-playlist-menu" id={PLAYLIST_MENU_ID}>
        ...
        {trackList.length ? (
          ...
          <div className="mp-playlist-menu__no-tracks">
            <p className="mp-playlist-menu__message">
              No tracks have been added yet.
            </p>
            <p className="mp-playlist-menu__message">
              Click the icon above or drop files anywhere on the screen.
            </p>
          </div>
        )}
      </SliderMenu>
